...
async fn test_play_upload_session() -> anyhow::Result<()> {
    ...
    // Unauthorized
    let request = server.post_bytes(&format!("{upload_url}/test"), Bytes::copy_from_slice(&data));
    ...
    let request = server.post_bytes(
        &format!("{}/{}", upload_url, token),
...
//     {
//         let request = server.patch_bytes(&format!("{upload_url}/{token}"), next_chunk);
//
//         let resp = request.await;
//         resp.assert_status_ok();
//
